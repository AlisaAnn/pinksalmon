...
# Hatcheries color-coded squares
  ...
  #guides(fill = "none") +
#deleted above line to move legend to bottom
  ...
  geom_shadowtext(
    ...
    hjust = 0,
    vjust = 0,
    size = 6, #was 5 but making year larger
    #fontface = "bold",
    color = "black",
    bg.color = "white",
    bg.r = 0.1
  ...
  facet_wrap(~year, nrow = 2, axes = "all") +  #axes = "all", will hopefully add latitude
  ...
  guides(
    fill = guide_legend(
      #ncol = 1, #changed ncol to nrow to move legend to bottom
      nrow = 1,
      byrow = TRUE
    ),
    color = "none"
  ...
  theme(
    ...
    legend.position = "bottom", #when it was on left of upper plot legend.position = c(0.015, 0.95),
    legend.justification = c(0.5, 0.5), # it was = c(0, 1),
...
ggsave(
  ...
  units = "in",
  dpi = 1200 #originally it was 600 
)
